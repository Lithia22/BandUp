...
def get_questions(component: Optional[str] = None, set_number: Optional[int] = None, year: Optional[int] = None):
   try:
...
def delete_questions(component: Optional[str] = None, set_number: Optional[int] = None, year: Optional[int] = None):
   try:
       if not component or not set_number:
           raise HTTPException(status_code=400, detail="Component and set_number are required")
       
       query = supabase.table("questions").select("id").eq("component", component).eq("set_number", set_number)
       ...
       if not result.data:
           return {"message": f"No questions found for {component} set {set_number}" + (f" year {year}" if year else ""), "deleted_count": 0}
       
       delete_query = supabase.table("questions").delete().eq("component", component).eq("set_number", set_number)
       
       if year:
           delete_query = delete_query.eq("year", year)
       
       delete_query.execute()
       
       return {"message": f"Deleted {len(result.data)} questions from {component} set {set_number}" + (f" year {year}" if year else ""), "deleted_count": len(result.data)}
   except Exception as e:
       raise HTTPException(status_code=500, detail=str(e))
