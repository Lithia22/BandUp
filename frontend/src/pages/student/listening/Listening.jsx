...
import { Clock, FileText, RotateCcw, Play } from 'lucide-react'
import {
  SidebarProvider,
  SidebarInset,
  SidebarTrigger,
...
const storageKey = (setNumber) => `listening_quiz_${setNumber}`
    ...
    const initializeData = async () => {
      try {
        ...
        for (let i = 0; i < localStorage.length; i++) {
          ...
          if (key?.startsWith('listening_quiz_')) {
            try {
              const setNumber = key.replace('listening_quiz_', '')
              const data = JSON.parse(localStorage.getItem(key))
              if (data?.answers && Object.keys(data.answers).length > 0) {
                savedProgress[setNumber] = {
                  answeredCount: Object.values(data.answers).filter(
                    (v) => v != null
                  ).length,
            ...
            {!loading && !error && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 max-w-2xl">
                {sets.map((set) => {
                  ...
                  const answeredCount = hasProgress?.answeredCount || 0
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 mt-2 mb-4">
                        <span className="flex items-center gap-1 text-[10px] font-semibold text-[#151313]/50">
                          <Clock size={10} /> 50 mins
                        </span>
                        <span className="flex items-center gap-1 text-[10px] font-semibold text-[#151313]/50">
                          <FileText size={10} /> 30 questions
                        </span>
                      </div>
                      ...
                      {hasProgress && (
                        <div className="mb-4">
                          <div className="flex items-center justify-between text-[10px] font-semibold mb-1">
                            <span className="text-[#151313]/50">Progress</span>
