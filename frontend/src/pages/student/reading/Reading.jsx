    ...
    const initializeData = async () => {
      try {
        ...
        for (let i = 0; i < localStorage.length; i++) {
          ...
          if (key?.startsWith('reading_quiz_')) {
            try {
              const setNumber = key.replace('reading_quiz_', '')
  ...
  const handleStartSet = (setNumber, action = 'start') => {
    if (action === 'restart') {
      ...
      setProgress((prev) => {
        ...
        delete newProgress[setNumber]
        return newProgress
      })
    }
    navigate(`/reading/${setNumber}`)
  }
                    ...
                    >
                      ...
                      <div className="flex items-center gap-3 mt-2 mb-4">
                        <span className="flex items-center gap-1 text-[10px] font-semibold text-[#151313]/50">
                          <Clock size={10} /> 75 mins
                        </span>
                        <span className="flex items-center gap-1 text-[10px] font-semibold text-[#151313]/50">
                          <FileText size={10} /> 40 questions
                        </span>
                      </div>
