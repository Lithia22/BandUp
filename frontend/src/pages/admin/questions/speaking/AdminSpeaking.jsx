...
const getDrafts = () => {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (key?.startsWith('speaking_admin_draft_')) {
      try {
        const data = JSON.parse(localStorage.getItem(key))
        if (data?.timestamp)
          drafts.push({ id: key.replace('speaking_admin_draft_', ''), ...data })
      } catch {}
    }
  }
  return drafts
...
export default function AdminSpeaking() {
  ...
  // Delete by component + set_number + year — same as Writing, keeps other years safe
  const handleDeleteSet = async () => {
    try {
      await api.delete(
        `/admin/questions?component=speaking&set_number=${deleteDialog.id}&year=${deleteDialog.year}`
      )
      toast.success(`Set ${deleteDialog.id} (${deleteDialog.year}) deleted`)
      loadPublishedSets()
    } catch {
  ...
  const handleDeleteDraft = () => {
    localStorage.removeItem(`speaking_admin_draft_${deleteDialog.id}`)
    setDrafts(getDrafts())
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-white/50 text-xs font-medium">
                      Create, edit and manage speaking practice sets
                    </p>
                  </div>
                  <Button
                    onClick={() => navigate('/admin/speaking/editor')}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {drafts.map((draft) => (
                    <Card
                      ...
                      onClick={() =>
                        navigate(`/admin/speaking/editor?draft=${draft.id}`)
                      }
                    >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] font-semibold text-[#151313]/40">
                                {draft.year || '—'} •{' '}
                                {draft.booklets?.length || 0} booklets
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex gap-1">
                            <Button
                              ...
                              onClick={(e) => {
                                e.stopPropagation()
                                navigate(
                                  `/admin/speaking/editor?draft=${draft.id}`
                                )
                              }}
                            >
                              <Edit size={14} />
              ...
              {loading ? (
                ...
                <Card className="border-2 border-[#151313] rounded-2xl bg-white/50">
                  <CardContent className="p-8 text-center">
                    <Mic size={40} className="mx-auto mb-3 text-[#151313]/20" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {publishedSets.map((set) => (
                    <Card
                      key={`${set.set_number}_${set.year}`}
                      className="group cursor-pointer border-2 border-[#151313] rounded-2xl shadow-[4px_4px_0px_#151313] hover:shadow-[6px_6px_0px_#151313] hover:-translate-y-1 transition-all duration-200"
                      onClick={() =>
                        navigate(
                          `/admin/speaking/editor/${set.set_number}?year=${set.year}`
                        )
                      }
                    >
                      <CardContent className="p-5">
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="text-base font-black text-[#151313]">
                                Set {set.set_number} ({set.year})
                              </p>
                              <p className="text-[10px] font-semibold text-[#151313]/40">
                                {set.year} • {set.total_booklets || 0} booklets
                              </p>
                            </div>
                          </div>
                            ...
                            <Button
                              ...
                              onClick={(e) => {
                                ...
                                setDeleteDialog({
                                  ...
                                  id: set.set_number,
                                  year: set.year,
                                  label: `Set ${set.set_number} (${set.year})`,
                                })
                              }}
                            >
                              <Trash2 size={14} />
