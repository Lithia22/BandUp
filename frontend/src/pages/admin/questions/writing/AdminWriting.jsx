...
import { SquarePen, Plus, Edit, Trash2 } from 'lucide-react'
...
const getDrafts = () => {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (key?.startsWith('writing_admin_draft_')) {
      try {
        const data = JSON.parse(localStorage.getItem(key))
        if (data?.timestamp)
          drafts.push({ id: key.replace('writing_admin_draft_', ''), ...data })
      } catch {}
    }
  }
  return drafts
...
export default function AdminWriting() {
  ...
  const handleDeleteSet = async () => {
    try {
      await api.delete(
        `/admin/questions?component=writing&set_number=${deleteDialog.id}&year=${deleteDialog.year}`
      )
  ...
  const handleDeleteDraft = () => {
    localStorage.removeItem(`writing_admin_draft_${deleteDialog.id}`)
    setDrafts(getDrafts())
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-white/50 text-xs font-medium">
                      Create, edit and manage writing practice sets
                    </p>
                  </div>
                  <Button
                    onClick={() => navigate('/admin/writing/editor')}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {drafts.map((draft) => (
                    <Card
                      ...
                      onClick={() =>
                        navigate(`/admin/writing/editor?draft=${draft.id}`)
                      }
                    >
                          ...
                          <div className="flex gap-1">
                            <Button
                              ...
                              onClick={(e) => {
                                e.stopPropagation()
                                navigate(
                                  `/admin/writing/editor?draft=${draft.id}`
                                )
                              }}
                            >
                              <Edit size={14} />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {publishedSets.map((set) => (
                    <Card
                      ...
                      onClick={() =>
                        navigate(
                          `/admin/writing/editor/${set.set_number}?year=${set.year}`
                        )
                      }
                    >
                      <CardContent className="p-5">
