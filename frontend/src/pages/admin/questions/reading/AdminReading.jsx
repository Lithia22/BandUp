...
const getDrafts = () => {
  ...
  for (let i = 0; i < localStorage.length; i++) {
    ...
    if (key?.startsWith('reading_draft_')) {
      try {
        const data = JSON.parse(localStorage.getItem(key))
        drafts.push({
          id: key.replace('reading_draft_', ''),
          ...data,
        })
      } catch (e) {}
    }
...
export default function AdminReading() {
  ...
  const handleDeleteSet = async () => {
    try {
      await api.delete(
        `/admin/questions?component=reading&set_number=${deleteDialog.id}`
      )
  ...
  const handleDeleteDraft = () => {
    localStorage.removeItem(`reading_draft_${deleteDialog.id}`)
    setDrafts(getDrafts())
                ...
                <div className="flex items-start justify-between">
                  <div>
                    ...
                    <p className="text-white/50 text-xs font-medium">
                      Create, edit and manage reading practice sets
                    </p>
                  </div>
                  <Button
                    onClick={() => navigate('/admin/reading/editor')}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {drafts.map((draft) => (
                    <Card
                      ...
                      onClick={() =>
                        navigate(`/admin/reading/editor?draft=${draft.id}`)
                      }
                    >
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] font-semibold text-[#151313]/40">
                                {draft.partsCompleted || 0}/7 parts completed
                              </p>
                            </div>
                          </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {publishedSets.map((set) => (
                    <Card
                      ...
                      className="group cursor-pointer border-2 border-[#151313] rounded-2xl shadow-[4px_4px_0px_#151313] hover:shadow-[6px_6px_0px_#151313] hover:-translate-y-1 transition-all duration-200"
                      onClick={() =>
                        navigate(`/admin/reading/editor/${set.set_number}`)
                      }
                    >
                      <CardContent className="p-5">
                        <div className="flex items-start justify-between mb-3">
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-[10px] font-semibold text-[#151313]/40">
                                {set.year} • {set.total_questions || 40}{' '}
                                questions
                              </p>
                            </div>
                          </div>
                          ...
                          <div className="flex gap-1">
                            <Button
                              ...
                              onClick={(e) => {
                                e.stopPropagation()
                                navigate(
                                  `/admin/reading/editor/${set.set_number}`
                                )
                              }}
                            >
                              <Edit size={14} />
